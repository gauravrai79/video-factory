...
def judge(channel, cast_chars: list, idea: dict[str, Any], scenes: list[dict]) -> WriterResult:
    ...
    system = (
        "You are an experienced showrunner and YouTube-comedy editor for an animated Hindi comedy "
        "series. You score scripts against PRODUCEABLE quality — 'would this make a good short that "
        "performs?' — NOT film-festival perfection. You are honest and specific, but you use the full "
        "0-10 range correctly: a competent, produceable script is a 7-8, not a 4. Respond ONLY JSON.")
    user = (
        ...
        "Score each dimension 0-10 using THESE anchors — use the full range honestly, do NOT default low:\n"
        "  9-10 = exceptional, best-in-class\n"
        "  7-8  = strong — PRODUCEABLE as-is and would perform well (a solid, competent script lives HERE)\n"
        "  5-6  = workable but ONE real flaw to fix\n"
        "  3-4  = weak, a genuine structural problem\n"
        "  0-2  = broken or incoherent\n"
        "A script with a clear premise, a working comic engine, and a payoff you can see on screen "
        "should score 7-8 on MOST dimensions. Reserve 3-4 for real structural failures, NOT for "
        "'could be sharper'. Score for what a good animated Hindi comedy short needs, not a feature film.\n"
        "Dimensions:\n"
        "- hook: does scene 1 grab within the first ~5 seconds (a clear visual + a punchy line)?\n"
        "- narrative: causality (each scene follows from the last), rising stakes, and a resolution "
        "the audience can SEE (the plan visibly works on screen, not just narrated).\n"
        "- ending: does the finale pay off + land a sharp closing line?\n"
        ...
        "(The composite maps to: 75+ green-light · 60-74 good, one area to lift · <60 needs real revision.)\n"
